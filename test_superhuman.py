...
def get_args():
    parser = argparse.ArgumentParser(
        description='Tests a previously trained PNI 3DUNet model for parallel fiber affinity prediction')
    # I/O
    ...
    parser.add_argument('-zp', '--z-pad', type=int, default=8,
                        help="Number of voxels for mirror padding the z axis of the test volume. Required for " +
                             "eliminating the gray grid on the edges of the prediction resulting from the Gaussian" +
                             "blending. The volume will be padded by this amount on both ends.")
    parser.add_argument('-ns', '--n_slices', type=int, default=60,
                        help="Numer of slices per chunk of volume")
    parser.add_argument('-isl', '--i_slab', type=int, default=0,
                        help="id of chunk in volume for testing")
...
def load_volumes(args, model_input_size):
    ...
    start_slice = n_slices*args.i_slab
    print("On slice %d"%(start_slice))
    print("Loading %d slices of input volume..."%(n_slices))
    for i in range(len(in_path)):
        test_input[i] = np.array(h5py.File(in_path[i], 'r')['main'][start_slice:start_slice+n_slices,:,:], dtype=np.float32) / 255.0
        ...
        print("Input volume size: %d"%(test_input[i].nbytes))
        print("Memory usage:%d"%(process.memory_info().rss))
        print("Input type:", test_input[i].dtype)
    ...
    print("Finished creating image dataloader. Memory usage:%d"%(process.memory_info().rss)) 
...
def test(args, test_loader, result, weight, model, device, model_io_size):
    ...
    start_slice = args.i_slab*args.n_slices

    print('Starting prediction for slab starting at slice %d...'%(start_slice))
    start = time.time()
    with torch.no_grad():
        for i, (pos, volume, _, _, _) in enumerate(test_loader):
            ...
            print("Finished testing this volume")

    end = time.time()
    print("Finished entire volume.")
    ...
    for vol_id in range(len(result)):
        ...
        hf = h5py.File(args.output + 'aff_%04d.h5'%(start_slice), 'w')
        hf.create_dataset('main', data=data[:, z_pad:-z_pad, y_pad:-y_pad, x_pad:-x_pad], compression='gzip')
        hf.close()
